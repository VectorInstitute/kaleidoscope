...
logger = logging.getLogger("triton.opt_client")
...
def main():
    ...
    # Initialize the max_tokens array
    max_tokens = np.full_like(sequences, 5, dtype=np.int16)

    logger.info(f"Waiting for response...")
    with ModelClient(args.url, "opt6.7b_generation", init_timeout_s=args.init_timeout_s) as client:
        for req_idx in range(1, args.iterations + 1):
            logger.info(f"Sending request ({req_idx}).")
            result_dict = client.infer_batch(sequences, max_tokens)
