...
Usage:
    python -m metaseq.scripts.get_metadata --model_folder . (--world_size 2)
    $ ls .
    gpt2-merges.txt
    gpt2-vocab.json
...
logger = logging.getLogger("get_metadata")
...
def worker_main(cfg: MetaseqConfig):
    ...
    def _build_model(cfg, task):
        ...
        model = task.build_model(cfg.model).half()
        return fsdp_wrap(model)

    logger.info(f"load model from {cfg.common_eval.path} with suffix {cfg.checkpoint.checkpoint_suffix}")
...
def main():
    ...
    real_parser.add_argument(
            "--model_folder", action = 'store', 
            type = str, required = True,
            help ='reshard model folder path')
    real_parser.add_argument(
            "--output_path", action = 'store', type = str,
            default = "./shard_metadata.pkl",
            help ='reshard model folder path')
    real_parser.add_argument(
            "--bpe_files", action = 'store', type = str,
            default = "/scratch/ssd001/home/jba/OPT/",
            help ='GPT-2 BPE vocab and json location'
    )
    real_parser.add_argument(
            "--world_size", action = 'store', type = int,
            default = 0,
            help ='world size'
    )
    args = real_parser.parse_args()
    print(args)
    files = glob.glob(f"{args.model_folder}/reshard*.pt")
    ...
    BPE_MERGES = args.bpe_files + "/gpt2-merges.txt"
    BPE_VOCAB = args.bpe_files + "/gpt2-vocab.json"
    ...
    #cfg.distributed_training.distributed_world_size = WORLD_SIZE
    print(cfg)
