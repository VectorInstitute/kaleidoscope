...
def launch(model_instance_id: str, model_name: str, model_path: str) -> None:
    ...
    try:
        ssh_command = f"""ssh -i id_rsa_kscope_slurm -o 'StrictHostKeyChecking no'{Config.JOB_SCHEDULER_USER}@{Config.JOB_SCHEDULER_HOST} {Config.JOB_SCHEDULER_BIN} --action launch --model_type {model_name} --model_path {model_path} --model_instance_id {model_instance_id} --gateway_host {Config.GATEWAY_ADVERTISED_HOST} --gateway_port {Config.GATEWAY_PORT}"""
...
def verify_job_health(model_instance_id: str) -> bool:
    try:
        ssh_command = f"ssh -i id_rsa_kscope_slurm -o 'StrictHostKeyChecking no' {Config.JOB_SCHEDULER_USER}@{Config.JOB_SCHEDULER_HOST} {Config.JOB_SCHEDULER_BIN} --action get_status --model_instance_id {model_instance_id}"
