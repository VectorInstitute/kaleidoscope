...
def shutdown(model_instance_id: str) -> None:
    try:
        ssh_command = f"ssh {Config.JOB_SCHEDULER_USER}@{Config.JOB_SCHEDULER_HOST} python3 \
        {Config.JOB_SCHEDULER_BIN} --action shutdown --model_instance_id {model_instance_id}"
